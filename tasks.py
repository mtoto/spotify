...
# Task to create new tracks playlist
class spotify_new_tracks_playlist(luigi.Task):
    date = luigi.DateParameter(default = date.today())
    
    def run(self):
        
        playlists = my_playlists('1170891844')
        songs = all_playlist_tracks(playlists)
        res_code = create_new_tracks_playlist(songs)
    ...
    def output(self):
        client = S3Client(host = 's3.us-east-2.amazonaws.com')
        return S3Target('s3://myspotifydata/spotify_newtracks_%s.json' % 
                        (self.date.strftime('%Y-%m-%d') + '_' ), 
                        client=client)
