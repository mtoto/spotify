...
class spotify_morning_playlist(luigi.Task):
    ...
    def run(self):
        ...
        data = data.set_index('played_at').between_time('7:00','12:00')
        data = data[data.index > str(self.date)]
        # aggregate data
        songs = data['uri'].value_counts().nlargest(10).index.get_values().tolist()
        # make api call
        res_code = replace_tracks(songs)
        # write to file if succesful
        if (res_code == 201):
            with self.output().open('w') as out_file:
                json.dump(songs, out_file)
